...
- **Host watchdog** (`api/watchdog.py`): background daemon, interval 15s. Evacuation threshold is **per-failure-kind** so container-level issues get more patience than traffic-exposure ones (`check_mule_vpn` tags each result with `kind`: `ip_leak`→1, `probe_failed`/`container_missing`→3, `not_running`/`exited`→8, `restarting`→20). This stops false-positive evacuations when Docker's `unless-stopped` policy is mid-recovery but still pulls the kill cord fast if traffic is actually leaking. State is per-mule (`consecutive_failures`, `failure_threshold`, `evacuated`) and cleaned up when a mule disappears from `list_mules`. Evacuation migrates active/waiting torrents to a healthy target mule before killing the compromised one.
