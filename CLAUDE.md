...
5. **Security:** Never log or expose PrivateKeys/Credentials. Use environment variables for mule credentials. Stored OpenVPN passwords **and VPN config bodies** (WireGuard private keys, inline OpenVPN keys) are encrypted at rest via `api/crypto.py` — **scrypt**-derived key from `SMG_SECRET_KEY` + `SMG_SECRET_SALT`, with a `MultiFernet` chain so legacy SHA-256 ciphertext still decrypts and `init_db` rotates it. Strings via `encrypt`/`decrypt`, BLOBs via `encrypt_bytes`/`decrypt_bytes`; keep encrypt/decrypt inside `api/database.py` so callers only ever see plaintext. Writing a secret with no key **raises** rather than silently storing plaintext.
...
- **Testing:** 367 backend tests + 59 frontend tests. Never let a change reduce the count; branding changes must update `tests/`.
